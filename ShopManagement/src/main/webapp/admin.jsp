...
<body class="easyui-layout">
	<div data-options="region:'north',border:false"
		style="height: 60px; background: #808040; padding: 10px">
		<div align="left">
			<div style="font-family: Microsoft YaHei; font-size: 16px;">时尚购物后台管理系统</div>
		</div>
		<div align="right">
	...
	<div data-options="region:'west',split:true,title:'功能菜单'"
		style="width: 180px; background: #AFAF61">
		<ul id="tt"></ul>
	</div>
	<div data-options="region:'south',border:false"
		style="height: 50px; background: #AFAF61; padding: 10px; text-align: center">powered
		by miaoyong</div>
	...
	<script type="text/javascript">
		...
		$('#tt').tree({
			onClick : function(node) {
				...
				// 添加内容
				else if ("创建用户" == node.text) {
					if ($('#tabs').tabs('exists', '创建用户')) {
						$('#tabs').tabs('select', '创建用户');
					} else {
						$('#tabs').tabs('add', {
							title: node.text,
							href: 'createuser.jsp',
